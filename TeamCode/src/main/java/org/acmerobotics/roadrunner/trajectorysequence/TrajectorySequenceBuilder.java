...
public class TrajectorySequenceBuilder {
	...
	private double lastDurationTrack;
	private double lastDisplacementTrack;

	public TrajectorySequenceBuilder(final Pose2d startPose, final Double startTangent, final TrajectoryVelocityConstraint baseVelConstraint, final TrajectoryAccelerationConstraint baseAccelConstraint, final double baseTurnConstraintMaxAngVel, final double baseTurnConstraintMaxAngAccel) {
		...
		lastDurationTrack = 0.0;
		lastDisplacementTrack = 0.0;
	}

	public TrajectorySequenceBuilder(final Pose2d startPose, final TrajectoryVelocityConstraint baseVelConstraint, final TrajectoryAccelerationConstraint baseAccelConstraint, final double baseTurnConstraintMaxAngVel, final double baseTurnConstraintMaxAngAccel) {
	...
	public TrajectorySequenceBuilder lineTo(final Vector2d endPosition, final TrajectoryVelocityConstraint velConstraint, final TrajectoryAccelerationConstraint accelConstraint) {
	...
	public TrajectorySequenceBuilder lineToConstantHeading(final Vector2d endPosition, final TrajectoryVelocityConstraint velConstraint, final TrajectoryAccelerationConstraint accelConstraint) {
	...
	public TrajectorySequenceBuilder lineToLinearHeading(final Pose2d endPose, final TrajectoryVelocityConstraint velConstraint, final TrajectoryAccelerationConstraint accelConstraint) {
	...
	public TrajectorySequenceBuilder lineToSplineHeading(final Pose2d endPose, final TrajectoryVelocityConstraint velConstraint, final TrajectoryAccelerationConstraint accelConstraint) {
	...
	public TrajectorySequenceBuilder strafeTo(final Vector2d endPosition, final TrajectoryVelocityConstraint velConstraint, final TrajectoryAccelerationConstraint accelConstraint) {
	...
	public TrajectorySequenceBuilder forward(final double distance, final TrajectoryVelocityConstraint velConstraint, final TrajectoryAccelerationConstraint accelConstraint) {
	...
	public TrajectorySequenceBuilder back(final double distance, final TrajectoryVelocityConstraint velConstraint, final TrajectoryAccelerationConstraint accelConstraint) {
	...
	public TrajectorySequenceBuilder strafeLeft(final double distance, final TrajectoryVelocityConstraint velConstraint, final TrajectoryAccelerationConstraint accelConstraint) {
	...
	public TrajectorySequenceBuilder strafeRight(final double distance, final TrajectoryVelocityConstraint velConstraint, final TrajectoryAccelerationConstraint accelConstraint) {
	...
	public TrajectorySequenceBuilder splineTo(final Vector2d endPosition, final double endHeading, final TrajectoryVelocityConstraint velConstraint, final TrajectoryAccelerationConstraint accelConstraint) {
	...
	public TrajectorySequenceBuilder splineToConstantHeading(final Vector2d endPosition, final double endHeading, final TrajectoryVelocityConstraint velConstraint, final TrajectoryAccelerationConstraint accelConstraint) {
	...
	public TrajectorySequenceBuilder splineToLinearHeading(final Pose2d endPose, final double endHeading, final TrajectoryVelocityConstraint velConstraint, final TrajectoryAccelerationConstraint accelConstraint) {
	...
	public TrajectorySequenceBuilder splineToSplineHeading(final Pose2d endPose, final double endHeading, final TrajectoryVelocityConstraint velConstraint, final TrajectoryAccelerationConstraint accelConstraint) {
	...
	private TrajectorySequenceBuilder addPath(final AddPathCallback callback) {
		...
		final Trajectory builtTrack = currentTrajectoryBuilder.build();

		final double durationDifference     = builtTrack.duration() - lastDurationTrack;
		final double displacementDifference = builtTrack.getPath().length() - lastDisplacementTrack;

		lastPose = builtTrack.end();
		currentDuration += durationDifference;
		currentDisplacement += displacementDifference;

		lastDurationTrack = builtTrack.duration();
		lastDisplacementTrack = builtTrack.getPath().length();
	...
	public TrajectorySequenceBuilder setConstraints(final TrajectoryVelocityConstraint velConstraint, final TrajectoryAccelerationConstraint accelConstraint) {
	...
	public TrajectorySequenceBuilder turn(final double angle, final double maxAngVel, final double maxAngAccel) {
		...
		final MotionProfile turnProfile = MotionProfileGenerator.generateSimpleMotionProfile(new MotionState(lastPose.getHeading(), 0.0, 0.0, 0.0), new MotionState(lastPose.getHeading() + angle, 0.0, 0.0, 0.0), maxAngVel, maxAngAccel);
	...
	private void pushPath() {
		if (null != currentTrajectoryBuilder) {
			final Trajectory builtTrack = currentTrajectoryBuilder.build();
			sequenceSegments.add(new TrajectorySegment(builtTrack));
		}
	...
	public TrajectorySequence build() {
		...
		final List <TrajectoryMarker> globalMarkers = convertMarkersToGlobal(sequenceSegments, temporalMarkers, displacementMarkers, spatialMarkers);
		projectGlobalMarkersToLocalSegments(globalMarkers, sequenceSegments);

		return new TrajectorySequence(sequenceSegments);
	}

	private List <TrajectoryMarker> convertMarkersToGlobal(final List <SequenceSegment> sequenceSegments, final List <TemporalMarker> temporalMarkers, final List <DisplacementMarker> displacementMarkers, final List <SpatialMarker> spatialMarkers) {
		...
		for (final DisplacementMarker marker : displacementMarkers) {
			final double time = displacementToTime(sequenceSegments, marker.getProducer().produce(currentDisplacement));
	...
	private void projectGlobalMarkersToLocalSegments(final List <TrajectoryMarker> markers, final List <SequenceSegment> sequenceSegments) {
		...
		for (final SequenceSegment segment : sequenceSegments) {
			while (markerIndex < markers.size()) {
				...
				segment.getMarkers().add(new TrajectoryMarker(Math.max(0.0, marker.getTime()) - timeOffset, marker.getCallback()));
				++ markerIndex;
			}
		...
		for (final SequenceSegment segment : sequenceSegments) {
			if (segment instanceof TrajectorySegment) {
				...
				if (currentDisplacement + segmentLength > s) {
					...
					final double timeInSegment = motionProfileDisplacementToTime(thisSegment.getTrajectory().getProfile(), target);
