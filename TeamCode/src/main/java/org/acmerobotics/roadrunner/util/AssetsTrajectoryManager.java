...
public enum AssetsTrajectoryManager {
	...
	public static @Nullable TrajectoryGroupConfig loadGroupConfig() {
		try {
			final InputStream inputStream = AppUtil.getDefContext().getAssets().open("trajectory/" + TrajectoryConfigManager.GROUP_FILENAME);
	...
	public static @Nullable TrajectoryConfig loadConfig(final String name) {
		try {
			final InputStream inputStream = AppUtil.getDefContext().getAssets().open("trajectory/" + name + ".yaml");
