...
public class Right2 extends StructuralLinearMode {
	...
	public void linear() {
		...
		final Trajectory get_sample_1 = drive.trajectoryBuilder(RightSuspend).lineToLinearHeading(RightGetFirstSample).build();
		final Trajectory get_sample_2 = drive.trajectoryBuilder(RightGetFirstSample).lineToLinearHeading(RightGetSecondSample).build();

		final TrajectorySequence get_sample_suspend_1 = drive.trajectorySequenceBuilder(RightGetSecondSample).lineToSplineHeading(GetSample.plus(new Pose2d(0, - 5))).build();
		final TrajectorySequence get_sample_suspend_2 = drive.trajectorySequenceBuilder(RightSuspend).lineToSplineHeading(GetSample.plus(new Pose2d(0, - 5))).build();

		final TrajectorySequence suspend_got_sample_1 = drive.trajectorySequenceBuilder(GetSample).lineToLinearHeading(RightSuspend.plus(new Pose2d(5, 5))).build();
		final TrajectorySequence suspend_got_sample_2 = drive.trajectorySequenceBuilder(GetSample).lineToLinearHeading(RightSuspend.plus(new Pose2d(10, 5))).build();

		final Trajectory park = drive.trajectoryBuilder(RightSuspend.plus(new Pose2d(10, 5)).plus(new Pose2d(0, - 5.1))).lineToLinearHeading(GetSample).build();

		final Action pre_suspend             = utils.integralLiftUpPrepare().liftSuspendHighPrepare().pack();
		final Action suspend_and_pre_get     = utils.liftSuspendHigh().waitMs(300).openClip().waitMs(100).liftDown().integralIntakes().scaleOperate(scaleGetPosition).pack();
		final Action intake                  = utils.displayArms().waitMs(600).closeClaw().waitMs(300).integralIntakesEnding().pack();
		final Action decant_intake           = utils.openClaw().waitMs(100).closeClaw().waitMs(100).openClaw().waitMs(200).armsToSafePosition().decant().pack();
		final Action get_suspend_and_prepare = utils.boxRst().addAction(SimpleDriveOp.build(0, - 0.2, 0)).waitMs(800).closeClip().waitMs(1000).integralLiftUpPrepare().liftSuspendHighPrepare().pack();
		final Action suspend                 = utils.liftSuspendHigh().waitMs(300).openClip().waitMs(100).liftDown().pack();
		final Action pre_park                = utils.liftSuspendHigh().waitMs(300).openClip().waitMs(100).liftDown().integralIntakes().pack();
