...
import static org.firstinspires.ftc.teamcode.roadrunner.drive.DriveConstants.LOGO_FACING_DIR;
...
import static org.firstinspires.ftc.teamcode.roadrunner.drive.DriveConstants.USB_FACING_DIR;
...
public class SampleTankDrive extends TankDrive {
	...
	public SampleTankDrive(final HardwareMap hardwareMap) {
		...
		final IMU.Parameters parameters = new IMU.Parameters(new RevHubOrientationOnRobot(LOGO_FACING_DIR, USB_FACING_DIR));
		...
		trajectorySequenceRunner = new TrajectorySequenceRunner(follower, HEADING_PID, batteryVoltageSensor, new ArrayList <>(), new ArrayList <>(), new ArrayList <>(), new ArrayList <>());
	}
	...
	public TrajectorySequenceBuilder trajectorySequenceBuilder(final Pose2d startPose) {
		return new TrajectorySequenceBuilder(startPose, VEL_CONSTRAINT, accelConstraint, MAX_ANG_VEL, MAX_ANG_ACCEL);
	}
	...
	public void setWeightedDrivePower(final Pose2d drivePower) {
		...
		if (1 < Math.abs(drivePower.getX()) + Math.abs(drivePower.getHeading())) {
			// re-normalize the powers according to the weights
			final double denom = VX_WEIGHT * Math.abs(drivePower.getX()) + OMEGA_WEIGHT * Math.abs(drivePower.getHeading());
	...
	public List <Double> getWheelPositions() {
		...
		for (final DcMotorEx leftMotor : leftMotors) {
			leftSum += encoderTicksToInches(leftMotor.getCurrentPosition());
		}
		for (final DcMotorEx rightMotor : rightMotors) {
	...
	@Override
	public void setMotorPowers(final double v, final double v1) {
		for (final DcMotorEx leftMotor : leftMotors) {
	...
	public static TrajectoryVelocityConstraint getVelocityConstraint(final double maxVel, final double maxAngularVel, final double trackWidth) {
		return new MinVelocityConstraint(Arrays.asList(new AngularVelocityConstraint(maxAngularVel), new TankVelocityConstraint(maxVel, trackWidth)));
	}
